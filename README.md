...
ClassVote is a real-time, interactive web application where users create or join voting sessions to collectively provide feedback, typically for presentations. Participants must set a unique nickname for the session before they can engage. This nickname cannot be changed later. Participants can cast "like" or "dislike" votes, and a live leaderboard tracks the scores. Session administrators have controls to manage the feedback rounds, sound settings, results visibility, presenter queues, kick participants, and end sessions.
            ...
            match /sessions/{sessionId} {
              ...
              allow update: if request.auth != null && resource.data.sessionEnded == false && (
                              ...
                              (resource.data.adminUid == request.auth.uid &&
                                ...
                                request.resource.data.sessionType == resource.data.sessionType && // sessionType immutable
                                (
                                  // Participants map itself is not directly modified by these general admin actions (except kick)
                                  // So for these specific admin toggles, participants map should be unchanged from these actions
                                  (
                                  ...
                                  (
                                    ...
                                    // Check if the admin's own participant entry is preserved if it existed
                                    (request.auth.uid in resource.data.participants ? request.auth.uid in request.resource.data.participants : true) &&
                                    // Check that number of participants decreased by one (or more, if admin client batch-kicks, though UI supports one by one)
                                    request.resource.data.participants.keys().size() < resource.data.participants.keys().size() &&
                                    // Verify that other fields are not being changed during a kick operation
                                    request.resource.data.isRoundActive == resource.data.isRoundActive &&
                                    request.resource.data.likeClicks == resource.data.likeClicks &&
                                    request.resource.data.dislikeClicks == resource.data.dislikeClicks &&
                                    request.resource.data.sessionEnded == resource.data.sessionEnded &&
                              ...
                              (
                                ...
                                request.resource.data.participants == resource.data.participants && // participant map itself not changed by voting
                                request.resource.data.presenterQueue == resource.data.presenterQueue &&
                                request.resource.data.currentPresenterIndex == resource.data.currentPresenterIndex &&
                                request.resource.data.currentPresenterName == resource.data.currentPresenterName
                              ) ||
                              ...
                              (
                                ...
                                // Ensure other key session fields are not changed during nickname set
                                request.resource.data.adminUid == resource.data.adminUid &&
                                request.resource.data.isRoundActive == resource.data.isRoundActive &&
                                request.resource.data.likeClicks == resource.data.likeClicks &&
                                request.resource.data.dislikeClicks == resource.data.dislikeClicks &&
